...
const RentCar = () => {
    ...
    const handleBookNow = (carId) => {
        if (!isAuthenticated) return alert("Please log in to book a car.");
        ...
        <>
            ...
            <div className="offers-container" style={{ flex: 1 }}>
                <div className="offers-header">
                    <h2 className="offers-title">{cars.length > 0 ? "Search Results" : "Renr Car"}</h2>
                </div>

                {error && <p className="offers-error" style={{ color: "red", textAlign: "center" }}>{error}</p>}
                {loading ? (
                ...
                {showEditModal && userRole === "admin" && (
                    <EditCarModal carToEdit={carToEdit} handleEditSubmit={handleEditSubmit} closeModal={closeModal} />
                )}
            </div>
            <Footer2 />
        </>
